...
function GameclientKernel:registerPlayHandlers()
	...
	{	
     ...
     --发牌
        {
            mainCmdID=MDM_GF_GAME, 
            subCmdID = OxqkbnewDefine.SUB_S_SEND_CARD, 
        ...
        {
            mainCmdID=MDM_GF_GAME, 
                subCmdID = OxqkbnewDefine.SUB_S_USER_OPENCARD, 
            responseHandler = handler(self, self.OnSocketSubUserOpenCard)
        },
        --返回所有的卡牌
        {
            mainCmdID=MDM_GF_GAME, 
            subCmdID = OxqkbnewDefine.SUB_S_USER_OPEN, 
...
function GameclientKernel:OnSocketSubPlayerOpen(Params)
    self.App.EventCenter:dispatchEvent({
        name = OxqkbnewDefine.GAME_PLAYER_OPEN,
        para = Params,
    }) 
    --dump(Params)
end  
...
function GameclientKernel:OnSocketSubGameScene(Params)
    self.App.EventCenter:dispatchEvent({
            name = OxqkbnewDefine.GAME_SCENCE,
            para = Params,
        }) 
    --dump(Params)
end  
...
function GameclientKernel:OnSocketSubGameStart(Params) 
        self.App.EventCenter:dispatchEvent({
            name = OxqkbnewDefine.GAME_START,
            para = Params,
        })  
...
function GameclientKernel:OnSocketSubGameAddScore(Params)
     self.App.EventCenter:dispatchEvent({
            name = OxqkbnewDefine.GAME_ADD_SCORE,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubPlayerExit(Params)
    --print("玩家强制退出Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxqkbnewDefine.GAME_PLAYER_EXIT,
            para = Params,
        })
...
--摊牌
function GameclientKernel:OnSocketSubUserOpenCard(Params)
    print("开牌消息")
    self.App.EventCenter:dispatchEvent({
        name = OxqkbnewDefine.GAME_USER_OPEN_CAED,
            para = Params,
        }) 
--    for k,v in pairs(Params) do
--        print(k,v)
...
function GameclientKernel:OnSocketSubOpenCard(Params)
    print("开牌Socket")
    self.App.EventCenter:dispatchEvent({
        name = OxqkbnewDefine.GAME_OPEN_CARD,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubCallBanker(Params)
    --print("叫庄Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxqkbnewDefine.GAME_CALL_BANKER,
            para = Params,
        })
...
function GameclientKernel:OnSocketSendCard(Params)
    --print("发牌Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxqkbnewDefine.GAME_SEND_CARD,
            para = Params,
        })
