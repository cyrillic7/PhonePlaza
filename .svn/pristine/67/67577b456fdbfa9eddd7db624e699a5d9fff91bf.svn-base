...
function CardControl:SetOXCardData(CardData,count)
    self.cbOXCardCount = count
    for i=1,count do
        self.Shoot[i]=false
        self.OXCarddata=CardData[i]
    end 
end
...
--设置是否显示牌
function CardControl:SetShow(isshow,count)
    -- body
    self.ShowCard=isshow
    if not count then
        count = table.getn(self.PokerCards)
    end
