...
local PokerCard = class("PokerCard", function()
	return ccs.Armature:create("AnimationCard")
end)
...
function PokerCard:ctor(args)
	...
	if args.clickHandler then
		--local cardBg  = ccui.ImageView:create():addTo(self)

		--cardBg:loadTexture("ys9zhang/u_game_bg_cardb.png")
		--cardBg:setTouchEnabled(true)
    	--cardBg:setSwallowTouches(true)
   		--点击事件
		--cardBg:addTouchEventListener(handler(self, self.onClickCard))
...
function PokerCard:addTouchListener()
	local cardBg  = ccui.ImageView:create():addTo(self)
...
function PokerCard:onClickCard(sender,touchType)
	if self.clickHandler then
		self.clickHandler(sender,touchType)
	end
end
...
function PokerCard:dealBaccaratCard( args)
	local actionArray = {}
	self.isBanker = args.isBanker
	self:setPosition(args.startPos)
	local cardScale = self:getScale()
	--延迟发牌
	if args.delay ~= nil then
	...
	--计算运动时间
	local _mt = cc.MoveTo:create(0.3, args.endPos)
	table.insert(actionArray,_mt)
	--是否在最后翻开
	if args.isTurnOpen ~= nil and args.isTurnOpen then
		--先盖住
	...
	if args.moveEndHandler then
		self.moveEndHandler = args.moveEndHandler
		local _moveEnd = cc.CallFunc:create(handler(self, self.cardMoveEnd))
		table.insert(actionArray,_moveEnd)
	end
...
function PokerCard:cardMoveEnd()
	if self.moveEndHandler then
		self.moveEndHandler(self.isBanker,self)
	end
end
