...
function ErRenLandScene:ctor(args)
    ...
    --audio.preloadSound("errenland/audio/ready.mp3") 
...
function ErRenLandScene:InitUnits()
    ...
    local Gamtableb=display.newSprite("errenland/u_game_table.jpg")
    ...
    --读取json 文件
    self.jsonnode = cc.uiloader:load("errenland/GameView.json")
...
function ErRenLandScene:InitUserCardControl()
    ...
    local CardPos=
    {
      cc.p(display.cx-40,450),--0号位置 对家485
      cc.p(display.cx-40,300)--1号位置 对家
    }
    --用户出牌的间距
    local CardDistance=
    {
...
function ErRenLandScene:OnHintHandCard()
    ...
    local szSoundName="errenland/audio/selectCard.mp3"
...
function ErRenLandScene:OnGameEndViewClose(evt)
    ...
    local type=evt.type
    if type==true then
        self.ButtonStart:setPositionY(display.cy-80)
    else
        self.ButtonStart:setPositionY(display.cy-50)
    end
...
function ErRenLandScene:OnGameScenceMsg(evt)
    ...
    if self.ClientKernel.cbGameStatus==ErRenLandDefine.GS_T_PLAY then
        ...
        self:SetTopViewBackcardstr(Type)
...
function ErRenLandScene:OnSubGameStatrt(evt)
    ...
    local szSoundName="errenland/audio/GAME_START.mp3"
    SoundManager:playMusicEffect(szSoundName, false, false)
end
...
function ErRenLandScene:OnSubBankerInfo(evt)
    ...
    if self.m_wBankerUser~=65535 then
        ...
        SoundManager:playMusicEffect("errenland/audio/BANKER_INFO.mp3", false, false)
    end
...
function ErRenLandScene:OnSubOutCard(evt)
    ...
    print("11111111111111111111111----"..self.m_wCurrentUser)
    ...
    if UserOutCardDb.wOutCardUser==self.BeiRangUser then
        if (self.HandCardCount[outviewid+1]-self.m_RangpaiCount)<=4 and (self.HandCardCount[outviewid+1]-self.m_RangpaiCount)>0 then
            ...
            local szSoundName="errenland/audio/COUNT_WARN.mp3"
            SoundManager:playMusicEffect(szSoundName, false, false)
        end
    else
...
function ErRenLandScene:OnSubPassCard(evt)
    ...
    if PassCardDb.wPassCardUser~=65535 then
        ...
        SoundManager:playMusicEffect("errenland/audio/"..strGender..name, false, false)
    end
...
print("游戏结束了")
    ...
    if meResutScore<0 then
        local szSoundName="errenland/audio/GAME_LOSE.mp3"
        SoundManager:playMusicEffect(szSoundName, false, false)
    end
    if meResutScore>0 then
        local szSoundName="errenland/audio/GAME_WIN.mp3"
        SoundManager:playMusicEffect(szSoundName, false, false)
    end
...
function ErRenLandScene:ShowRangpai(beirang,userCount)
    self.Rangduifang=cc.uiloader:seekNodeByName(self.jsonnode,"rangduifang")
    self.BeiRang=cc.uiloader:seekNodeByName(self.jsonnode,"ninbeirang")
    self.Rangduifang:hide()
    self.BeiRang:hide()
    if beirang==self.m_GameKernel:GetMeChairID() then
        self.BeiRang:show()
        self.Number1=cc.uiloader:seekNodeByName(self.BeiRang,"number1")
        self.Number2=cc.uiloader:seekNodeByName(self.BeiRang,"number2")
        self.Number1:setString(tostring(userCount))
        self.Number2:setString(tostring(userCount))
        self.OtheUserCardNum2:setString(tostring(0))
        --todo
    else
        self.Rangduifang:show()
        self.Number1=cc.uiloader:seekNodeByName(self.Rangduifang,"number1")
        self.Number2=cc.uiloader:seekNodeByName(self.Rangduifang,"number2")
        self.Number1:setString(tostring(userCount))
        self.Number2:setString(tostring(userCount))
        self.OtheUserCardNum2:setString(tostring(userCount))
    end
end
--隐藏让牌信息
...
function ErRenLandScene:ShowTopView(show,move)
    ...
    if show then 
        self.TopBack:show()
        local sendpos=cc.p(0,self.TopBack:getPositionY())
        if move==true then
            self.TopBack:setPositionX(-self.TopBack:getContentSize().width)
            local actionArray = {}
            local _mt = cc.MoveTo:create(0.16, sendpos)

            table.insert(actionArray,_mt)
            local _seq = cc.Sequence:create(actionArray)
            self.TopBack:runAction(_seq)
        else
            self.TopBack:setPosition(sendpos)
...
function ErRenLandScene:SetTopViewBackBeishu(str)
    self.TopBackbackDIbeishu=cc.uiloader:seekNodeByName(self.TopBack,"Image_dibeishu")
...
--上方信息设置底牌描述
function ErRenLandScene:SetTopViewBackcardstr(str)
    self.TopBackbackcardstrf=cc.uiloader:seekNodeByName(self.TopBack,"Image_DI")
...
function ErRenLandScene:BtStartGame()
   ...
   -- self.m_OthHandCardControl:SetOthHandCardData(9,card,cc.p(display.cx-492,display.cy+50))
    ...
    --SoundManager:playMusicEffect("errenland/audio/ready.mp3", false, false)
end
...
function ErRenLandScene:BtShezhi()
    ...
    local settingView = require("errenland.App.Kernel.SettingView").new()
...
function ErRenLandScene:onCleanup()
    ...
    display.removeSpriteFramesWithFile("errenland/gamecommon.plist","errenland/gamecommon.png")
    display.removeSpriteFrameByImageName("errenland/u_game_table.jpg") 
...
function ErRenLandScene:PalyCalllandSound(cbCallScore,chairID,Count)
    ...
    if self:GetCallLandStatus()==ErRenLandDefine.CSD_NORMAL then
        ...
        if cbCallScore==1 then 
            ...
            SoundManager:playMusicEffect("errenland/audio/"..strGender.."SCORE_1.mp3", false, false)
        else --不叫
            SoundManager:playMusicEffect("errenland/audio/"..strGender.."SCORE_NONE.mp3", false, false)   
        end
        --todo
    end
    if self:GetCallLandStatus()==ErRenLandDefine.CSD_SNATCHLAND then
        ...
        if cbCallScore==1 then 
            if Count==1 then
                SoundManager:playMusicEffect("errenland/audio/"..strGender.."SCORE_2.mp3", false, false)
            end
            if Count==2 then
                SoundManager:playMusicEffect("errenland/audio/"..strGender.."SCORE_3.mp3", false, false)
            end
            if Count==3 then
                SoundManager:playMusicEffect("errenland/audio/"..strGender.."SCORE_3.mp3", false, false)
            end
            if Count==4 then
                SoundManager:playMusicEffect("errenland/audio/"..strGender.."IAmLord.mp3", false, false)
            end
        else --不抢
            SoundManager:playMusicEffect("errenland/audio/"..strGender.."NOQIANG.mp3", false, false)   
        end
    end
...
function ErRenLandScene:PlayOutCardSound(chairID,carddata,cardcount)
    ...
    local strGender
    if cbGender==GENDER_FEMALE then --女性
        strGender="errenland/audio/GIRL/"
    end
    if cbGender==GENDER_MANKIND then --男性
        strGender="errenland/audio/BOY/"
    end
    ...
    if cbCardType==ErRenLandDefine.CT_THREE_TAKE_ONE then
        ...
        if cardcount<8 then
            ...
            SoundManager:playMusicEffect("errenland/audio/PLANE.mp3", false, false)
        end
    ...
    if cbCardType==ErRenLandDefine.CT_BOMB_CARD then
        ...
        SoundManager:playMusicEffect("errenland/audio/BOMB.mp3", false, false)
