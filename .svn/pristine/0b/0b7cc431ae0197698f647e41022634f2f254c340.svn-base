...
function Player:ctor(Json,Kernel)
    ...
    for var=1, OxNewDefine.GAME_PLAYER do
...
--    dump(evt.para)
    ...
    if ViewID==OxNewDefine.MYSELF_VIEW_ID then
        --头像框-自己
...
function Player:setCellScore(wChairID,score)
    ...
    if wChairID == OxNewDefine.MYSELF_VIEW_ID then 
...
function Player:setSouceTexture(vid,iszheng)
    ...
    self["TXT" .. vid]:setProperty("0", "oxnew/Font/" .. str ..  ".png", 23, 31, "0")
