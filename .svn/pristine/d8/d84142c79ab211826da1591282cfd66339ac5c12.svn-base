...
function GameclientKernel:registerPlayHandlers()
	...
	{	
     ...
     --发牌
        {
            mainCmdID=MDM_GF_GAME, 
            subCmdID = OxNewDefine.SUB_S_SEND_CARD, 
        ...
        {
            mainCmdID=MDM_GF_GAME, 
                subCmdID = OxNewDefine.SUB_S_AMDIN_COMMAND, 
            responseHandler = handler(self, self.OnSocketSubAmdinCommand)
        },
        --返回所有的卡牌
        {
            mainCmdID=MDM_GF_GAME, 
            subCmdID = OxNewDefine.SUB_S_USER_OPEN, 
...
function GameclientKernel:OnSocketSubPlayerOpen(Params)
    self.App.EventCenter:dispatchEvent({
        name = OxNewDefine.GAME_PLAYER_OPEN,
        para = Params,
    }) 
    --dump(Params)
end  
...
function GameclientKernel:OnSocketSubGameAddScore(Params)
     self.App.EventCenter:dispatchEvent({
            name = OxNewDefine.GAME_ADD_SCORE,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubPlayerExit(Params)
    --print("玩家强制退出Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxNewDefine.GAME_PLAYER_EXIT,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubAmdinCommand(Params)
    print("系统设置")
    self.App.EventCenter:dispatchEvent({
            name = OxNewDefine.GAME_CHANGE_OPEN,
            para = Params,
        }) 
    for k,v in pairs(Params) do
        print(k,v)
...
function GameclientKernel:OnSocketSubOpenCard(Params)
    print("开牌Socket")
    self.App.EventCenter:dispatchEvent({
        name = OxNewDefine.GAME_OPEN_CARD,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubAllCard(Params)
    print("换牌Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxNewDefine.GAME_ALL_CARD,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubCallBanker(Params)
    print("叫庄Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxNewDefine.GAME_CALL_BANKER,
            para = Params,
        }) 
end 
-- 发牌
function GameclientKernel:OnSocketSendCard(Params)
    print("发牌Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxNewDefine.GAME_SEND_CARD,
            para = Params,
        }) 
end
-- --结束
