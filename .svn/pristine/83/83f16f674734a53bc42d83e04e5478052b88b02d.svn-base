...
function SettingView:loadUI()
	self.mainWidget = GameUtil:widgetFromCocostudioFile("errenlandmatch/settingWidget")
...
function SettingView:onSoundVolumeChange(pSender,eventtype)
	if eventtype == 0 then
...
function SettingView:onMusicVolumeChange(pSender,eventtype)
	if eventtype == 0 then
		local  percent = pSender:getPercent()
		local   volume= percent/100.0
...
function SettingView:onClickOk(pSender,touchType)
