...
oxui.playSound = function(name,isLoop,t)
    ...
    local type = t
    local str = ".mp3" 
    if type == OxFlowersNewDefine.wav then
        type = ".wav"
    elseif type == OxFlowersNewDefine.m4a then
        type = ".m4a"
    else
        type = ".mp3"
    end
    local n = "oxflowersnew/ogg/" .. name .. type
    
    SoundManager:playMusicEffect(n,loop,false)    
end
 
oxui.cacheAnimationList = oxui.cacheAnimationList or {}
oxui.animPath = "oxflowersnew/animation/"
...
oxui.removeAll = function ()  
    ...
    --cc.SpriteFrameCache:getInstance():removeSpriteFramesFromTexture("oxnew/u_game_table.jpg")
