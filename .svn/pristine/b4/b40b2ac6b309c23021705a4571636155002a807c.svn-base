...
function GameLogic:GetTimes(cbCardData, cbCardCount)
    ...
    if  cbCardCount~=OxFlowersNewDefine.MAXCOUNT then
        return 0
    end
...
function GameLogic:GetCardType2(cbCardData , cbCardCount)
    ...
    --cbcarddatas = self:SortCardList(cbCardData,cbCardCount)
    print("获得牌型 -- 无花牛")
    dump(cbCardData)
    --转换数值
    local cbSortValue= {}
    ...
    self:SortCardList(cbSortValue,cbCardCount)

    local bsecondValue = cbSortValue[math.ceil(OxFlowersNewDefine.MAXCOUNT/2)]
    local bsamecount = 0
  
    for i=1,5 do
        if(bsecondValue == cbSortValue[i]) then
            bsamecount = bsamecount+1
        end
    end
    if bsamecount == 4 then
        print("四带一 -- 无花牛")
        return OxFlowersNewDefine.OX_FOUR_SAME
    end
  
    local smallcount = 0
    local wsmallnum = 0
    for i=1,5 do
        if(cbSortValue[i] < 5) then
            smallcount = smallcount+1
            wsmallnum = wsmallnum + cbSortValue[i]
        end
    end
    if smallcount == 5 and wsmallnum <= 10 then
        print("五小牛 ---------------- ccy")
        return OxFlowersNewDefine.OX_FOURKING
    end

    if(bsamecount==3) then
        if((bsecondValue ~= cbSortValue[4] and cbSortValue[4] == cbSortValue[5]) or (bsecondValue ~= cbSortValue[2] and cbSortValue[2] == cbSortValue[1])) then
            print("三带二 -- 无花牛")
            return OxFlowersNewDefine.OX_THREE_SAME
         end
    end
