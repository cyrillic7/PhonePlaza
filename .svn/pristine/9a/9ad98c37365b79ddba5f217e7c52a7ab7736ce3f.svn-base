...
oxui.playSound = function(name,isLoop,t)
    ...
    local type = t
    local str = ".mp3" 
    if type == OxsixChangeDefine.wav then
        type = ".wav"
    elseif type == OxsixChangeDefine.m4a then
        type = ".m4a"
    else
        type = ".mp3"
    end
    local n = "oxsixchangenew/ogg/" .. name .. type
    
    SoundManager:playMusicEffect(n,loop,false)    
end
 
oxui.cacheAnimationList = oxui.cacheAnimationList or {}
oxui.animPath = "oxsixchangenew/animation/"
...
oxui.removeAll = function ()  
    ...
    --cc.SpriteFrameCache:getInstance():removeSpriteFramesFromTexture("oxsixchangenew/u_game_table.jpg")
