...
function Player:ctor(Json,Kernel)
    ...
    self.playerList:addEventListener(function (s,e) if e == ccui.ListViewEventType.ONSELECTEDITEM_END  then self:pklistViewEvent(s) end end )
...
function Player:showUserInfo(tag) 
    ...
    self.userInfoWidget:updateUserInfo(self.sixHeros[tag],300,display.cy +200)
...
function Player:setSouceTexture(vid,iszheng)
    ...
    self["TXT" .. vid]:setProperty("0", "redninebattle/Font/" .. str ..  ".png", 23, 31, "0")
