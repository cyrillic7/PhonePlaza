local TableLockWidget = class("TableLockWidget")

function TableLockWidget:ctor(parentNode,callBack)
    if not parentNode then
        return
    end
    local node, width, height = cc.uiloader:load(WIDGET_TABLE_LOCK_CSB_FILE)
    if not node then
        return
    end
...
function TableLockWidget:buttonTouchEvent(btn)
    if btn then
