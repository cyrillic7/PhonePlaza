...
--    dump(evt.para)
    ...
    if ViewID==OxqkbnewDefine.MYSELF_VIEW_ID then
        --头像框-自己
...
function Player:playSelectBanderAnim(t)
    local bts = t
...
function Player:setCellScore(wChairID,score)
    ...
    if wChairID == OxqkbnewDefine.MYSELF_VIEW_ID then 
...
function Player:setSouceTexture(vid,iszheng)
    ...
    self["TXT" .. vid]:setProperty("0", "oxqkbnew/Font/" .. str ..  ".png", 23, 31, "0")
