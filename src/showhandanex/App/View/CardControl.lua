...
function CardControl:SetCardDataEx(carddata)
    print("SetCardDataEx:carddata == "..carddata)
    if carddata == 1 then
        return
    end

    self.Carddata[1]=CardData

    dump(self.PokerCards)
    for key, var in pairs(self.PokerCards) do
        if self.PokerCards[key] == 1 then
            print("key == "..key)
            print("self.PokerCards[key] == "..self.PokerCards[key])
            self.PokerCards[key] = carddata
            print("self.PokerCards[key]1 == "..self.PokerCards[key])
            break
        end
    end
...
function CardControl:OnClickOneCard(s,msgtype,x,y)
    ...
    elseif msgtype=="moved" then
        --print("moved-------ccy")
    elseif msgtype=="ended" then
        self.PokerCards[1]:cover()
        --print("ended-------ccy")
    end
...
--设置是否显示牌
function CardControl:SetShowEx(index,isshow)
    -- body
    self.ShowCard=isshow
    print("显示卡牌")
    print("self.PokerCards" .. table.getn(self.PokerCards))
