	...
	for i,v in ipairs(children) do
		...
		if childrenNode then
			...
			elseif "ListView" == clsName then
				...
				-- end
				-- add by xwm
				if uiNode.DIRECTION_HORIZONTAL == options.direction then
					size.width = size.width + options.itemMargin or 0
				elseif uiNode.DIRECTION_VERTICAL == options.direction then
					size.height = size.height + options.itemMargin or 0
				end
...
function CCSUILoader:createUINode(clsName, options, parent)
	...
	-- add positionPercent sizePercent 
	if parent ~= nil then
		if node then
			if options.positionType == 1 then
				local posTrans = parent:getAnchorPoint()
				local parentSize = parent:getContentSize()
				posTrans.x = posTrans.x * parentSize.width
				posTrans.y = posTrans.y * parentSize.height
				local parentSize = parent:getContentSize()
				node:setPosition(posTrans.x+parentSize.width*options.positionPercentX, 
									posTrans.y+parentSize.height*options.positionPercentY)
			end
			if options.sizeType == 1 then
				local parentSize = parent:getContentSize()
				node:setContentSize(parentSize.width*options.sizePercentX, parentSize.height*options.sizePercentY)
			end
		end
	end
