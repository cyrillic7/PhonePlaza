...
function CardControl:SetCardData(CardData,count)
    ...
    for key, var in pairs(CardData) do
        self.Carddata[key] = var
    end 
end
...
function CardControl:SetShootCard(cbCardData,cbCardCount)
    ...
    for  i=1,cbCardCount do  
        for j=1,OxtbNewDefine.MAXCOUNT do
            if self.Carddata[j]==cbCardData[i] then  
                self.PokerCards[j]:setSelect()
            end
        end
    end 
...
function CardControl:SetShootCardOX(cbCardData,cbCardCount)
 
    local bChangeStatus=false;
    --收起扑克
    self:setUnSelected()   
    for  i=3,OxtbNewDefine.MAXCOUNT do  
        for j=3,OxtbNewDefine.MAXCOUNT do
            if self.Carddata[j]==cbCardData[i] then  
                self.PokerCards[j]:setSelect()
            end
        end
...
function CardControl:GetShootCard(bCardData, bShoot)
    --变量定义
    local cbShootCount=0;
    ...
    for i=1,OxtbNewDefine.MAXCOUNT do
        --拷贝扑克
        if (self.Shoot==bShoot) then
            cbShootCount =cbShootCount +1
            bCardData[cbShootCount]=self.Carddata[i]
        end
    end
...
function CardControl:ResetCardPos()
    ...
    --    local CardCount = table.getn(self.PokerCards)
    --    local x = (display.width - self.CardPos.x) +(CardCount-1)*self.CardDistance
    --    x=x/2
    --    local y= self.CardPos.y
    --    for i=1,CardCount do
    --        self.PokerCards[i]:zorder(i)
...
--function CardControl:SetShootOXOpoker(cardData,count)
--    for var=1, count do
--    	
--    	cardData
--    end
