...
oxui.getUIByName = function(name)
	if name then
        return ccs.GUIReader:getInstance():widgetFromJsonFile(name)
	else
		print("name is nil")
	end 
end 
...
oxui.playSound = function(name,isLoop,t)
    ...
    local type = t
    local str = ".mp3" 
    if type == OxBattleDefine.wav then
        type = ".wav"
    elseif type == OxBattleDefine.m4a then
        type = ".m4a"
    else
        type = ".mp3"
    end
    local n = "oxbattle/ogg/" .. name .. type
    
    SoundManager:playMusicEffect(n,loop,false)    
end
 
oxui.cacheAnimationList = oxui.cacheAnimationList or {}
oxui.animPath = "oxbattle/animation/"
...
oxui.removeAll = function ()  
    ...
    --cc.SpriteFrameCache:getInstance():removeSpriteFramesFromTexture("oxbattle/u_game_table.jpg")
...
oxui.setAnimLayer = function(tag)
    if tag then
    	tag:show() 
        tag:setScale(0.3)
        local action={}
...
oxui.getTipLayer = function(tag)
    
    local function close()
        if tag:getChildByName("sit_tip") then
            local img = tag:getChildByName("sit_tip")
    ...
    if tag then 
      ...
      img:loadTexture("sit_tip.png",1)
      img:addTo(tag,2002)
      img:setPosition(display.cx,200)
      
      local a = cc.MoveTo:create(2,cc.p(display.cx,300))
      local b = cc.FadeTo:create(1,0)
      local f = cc.CallFunc:create(function() 
         close()
      end)
        img:runAction(cc.Sequence:create(a,b,f))
    end
