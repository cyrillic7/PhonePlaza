...
function PokerCard:getCardAnimation( ctype, number )
    ...
    if ctype and number then
        ...
        aniName = string.format("card_%d_%d",ctype,num)
    end
    return aniName
end
...
function PokerCard:doCardAnimation(args)
    ...
    if args.isMoveTo then
        ...
        _mt = cc.JumpTo:create(moveTime,args.endPos,100,1)--cc.MoveTo:create(moveTime,args.endPos)
    else
        _mt =cc.MoveTo:create(0,args.endPos)
    end
