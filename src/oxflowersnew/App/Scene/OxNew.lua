...
function OxNew:ctor(args)
    display.addSpriteFrames("oxflowersnew/UIGameIng.plist","oxflowersnew/UIGameIng.png")
    display.addSpriteFrames("oxflowersnew/talkWidgetPlist.plist","oxflowersnew/talkWidgetPlist.png")
    display.addSpriteFrames("oxflowersnew/settingWidgetPlist.plist","oxflowersnew/settingWidgetPlist.png")
    self.App=AppBaseInstanse.OxNewApp
    self:setNodeEventEnabled(true)
    self.gameState = OxFlowersNewDefine.SUB_S_GAME_START
    ...
    local gameAttribute =
        {
            wKindID=OxFlowersNewDefine.KIND_ID,
            wPlayerCount=OxFlowersNewDefine.GAME_PLAYER,
            dwClientVersion=OxFlowersNewDefine.VERSION_CLIENT,
            pszGameName=OxFlowersNewDefine.GAME_NAME,
        }
...
function OxNew:FreeAllData()
    ...
    for i=1, OxFlowersNewDefine.GAME_PLAYER do
...
function OxNew:InitUnits()
    ...
    --读取json 文件
    local node = oxui.getUIByName("oxflowersnew/oxfourLayer.json")
    self.node  = node 
    node:addTo(self)
...
function OxNew:InitHandCardControl()
    for i=1, OxFlowersNewDefine.GAME_PLAYER do
        ...
        if i == OxFlowersNewDefine.MYSELF_VIEW_ID then
...
function OxNew:DisPatchOneCardEnd(evt)
    oxui.playSound("SEND_CARD",false,OxFlowersNewDefine.wav)
...
function OxNew:ButtonMsg()
    ...
    self.btnBanker:onButtonClicked(function ()
        ...
        self:send(OxFlowersNewDefine.SUB_C_CALL_BANKER,CMD_C_CallBanker ,"CMD_C_CallBanker")
        self.BankerView:hide()
        self.clock:hide()
    end)
    ...
    self.btnTip:onButtonClicked(function ()
        ...
        local bCardValue = GameLogic:GetCardType2(cardData,OxFlowersNewDefine.MAXCOUNT,cardData)
        if bCardValue > 0 then
            self.handCardControl[OxFlowersNewDefine.MYSELF_VIEW_ID]:SetShootOXOpoker()
        else
            self.openView:hide()
            self:SetAnimCard(OxFlowersNewDefine.MYSELF_VIEW_ID,0)
            local bOX = 0
            CMD_C_OxCard = {bOX = bOX}
            self:send(OxFlowersNewDefine.SUB_C_OPEN_CARD,CMD_C_OxCard ,"CMD_C_OxCard")
        end
    end)
...
function OxNew:sendChangeMsgNo()
    ...
    self:send(OxFlowersNewDefine.SUB_C_CHANGE_CARD,CMD_C_CHANGE_CARD ,"CMD_C_ChangeCard")
...
function OxNew:sendOpenMsg()
    local cardData = self.cbHandCardData[self:GetMeChairID()+1]
    local bOX = 0
    if GameLogic:GetOxCard(cardData,OxFlowersNewDefine.MAXCOUNT) then
        bOX = 1
    else
        self.openView:hide()
    end
...
function OxNew:sendAddSource(index)
    ...
    self:send(OxFlowersNewDefine.SUB_C_ADD_SCORE,CMD_C_AddScore ,"CMD_C_AddScore")
end
...
function OxNew:OnGameScenceMsg(evt)
    ...
    if gameStatus == OxFlowersNewDefine.GS_TK_FREE then
        print("空闲状态")
        ...
        if self:getMyStatus() == US_SIT and self:getMyStatus() ~= US_PLAYING then
            ...
            if not self.t_Start then
                self.gameState = OxFlowersNewDefine.GS_TK_FREE
                self.timerCount = 10
                self.t_Start = oxui.schedule(function()self:updateState() end,OxFlowersNewDefine.TIME_INTERVAL,OxFlowersNewDefine.TIME_USER_START_GAME)
            end

            self.btnStart:show() 
        end
        --叫庄状态
    elseif gameStatus == OxFlowersNewDefine.GS_TK_CALL then
        self.btnStart:hide() 
        ...
        --更新状态
        for i=0,OxFlowersNewDefine.GAME_PLAYER - 1 do
        ...
        if m_wBankerUser == self:GetMeChairID() then
            if not self.t_Bander then
                self:stopAllTimer()
                self.gameState = OxFlowersNewDefine.SUB_S_CALL_BANKER
                self.timerCount = OxFlowersNewDefine.TIME_USER_CALL_BANKER
                self.t_Bander = oxui.schedule(function()
                    self:updateState()
                end ,
                OxFlowersNewDefine.TIME_INTERVAL,
                OxFlowersNewDefine.TIME_USER_CALL_BANKER)
            end
        else
            if not self.t_Bander then
                self:stopAllTimer()
                self.gameState = OxFlowersNewDefine.SUB_S_CALL_OTH
                self.timerCount = OxFlowersNewDefine.TIME_USER_CALL_BANKER
                self.t_Bander = oxui.schedule(function()
                    self:updateState()
                end ,
        ...
        --加注状态
    elseif gameStatus == OxFlowersNewDefine.GS_TK_SCORE then
 ...
 --       self.gameState = OxFlowersNewDefine.SUB_S_ADD_SCORE
--        if not self.t_Change then
--            self:stopAllTimer()
--            self.timerCount = OxFlowersNewDefine.TIME_USER_ADD_SCORE
--            self.t_Change = oxui.schedule(function()
--                self:updateState()
--            end ,
--            OxFlowersNewDefine.TIME_INTERVAL,
--            OxFlowersNewDefine.TIME_USER_ADD_SCORE)
--        end
        self.wBankerUser = m_wBankerUser
        dump(self.cbPlayStatus)

        --加注状态
    elseif gameStatus == OxFlowersNewDefine.GS_TK_PLAYING then
        ...
        self.gameState = OxFlowersNewDefine.SUB_S_USER_OPEN
        self:stopAllTimer()
        if not self.t_Open then 
            self.timerCount = OxFlowersNewDefine.TIME_USER_OPEN_ING
            self.t_Open = oxui.schedule(function()
                self:updateState()
            end ,
            OxFlowersNewDefine.TIME_INTERVAL,
            OxFlowersNewDefine.TIME_USER_OPEN_ING)
        end
    end
...
function OxNew:OnSubPlayerOpen(evt)
    ...
    for i=OxFlowersNewDefine.GAME_PLAYER + banker - 1,banker ,-1 do  
        local w = i%OxFlowersNewDefine.GAME_PLAYER 
        if self.cbPlayStatus[w]==OxFlowersNewDefine.USEX_PLAYING and w ~= self:GetMeChairID() then
            local wViewChairID =  self.wViewChairID[ w] 
            if self.cbHandCardData[w+1] then
                self.handCardControl[wViewChairID]:SetCardData(self.cbHandCardData[w+1],OxFlowersNewDefine.MAXCOUNT)
                local time = cc.DelayTime:create(t)
                t = t+1
                index = index + 1
                local fuc = cc.CallFunc:create(function()
                    self.handCardControl[wViewChairID]:SetShow(true)
                    local num = GameLogic:GetCardType2(self.cbHandCardData[w+1],OxFlowersNewDefine.MAXCOUNT)
                    if num == OxFlowersNewDefine.OX_THREE_SAME then
                        num = 13
                    end
                    if num == OxFlowersNewDefine.OX_FOUR_SAME then
                        num = 14
                    end
                    if num == OxFlowersNewDefine.OX_FOURKING then
                        num = 15
                    end
    ...
    local c = cc.CallFunc:create(function()
        self:stopAllTimer()
        self:send(OxFlowersNewDefine.SUB_C_OPEN_END)
    end )
...
function OxNew:OnSubGameOver(evt)
    ...
    function callFuck()
        ...
        self.gameState = OxFlowersNewDefine.GS_TK_FREE
        if not self.t_Start then
            self.timerCount = OxFlowersNewDefine.TIME_USER_START_GAME
            self.t_Start = oxui.schedule(function(t) self:updateState(OxFlowersNewDefine.TIME_USER_START_GAME - t) end ,OxFlowersNewDefine.TIME_INTERVAL,OxFlowersNewDefine.TIME_USER_START_GAME)
        end
    ...
    if self:IsNotLookonMode() then
        if pGameEnd.lGameScore[self:GetMeChairID()+1] > 0 then
            oxui.playSound("GAME_WIN",false,OxFlowersNewDefine.m4a)
            ...
            oxui.playSound("GAME_LOST",false,OxFlowersNewDefine.m4a)
    ...
    for  i=1,OxFlowersNewDefine.GAME_PLAYER do
        ...
        if self.cbPlayStatus[i-1]==OxFlowersNewDefine.USEX_PLAYING then
            if souce >= 0 then
...
function OxNew:OnSubGameStart(evt)
    ...
    oxui.playSound("GAME_START",false,OxFlowersNewDefine.m4a)
    ...
    if not self.t_AddSource then
        ...
        print("加分")
        self.timerCount = OxFlowersNewDefine.TIME_USER_ADD_SCORE
        self.t_AddSource = oxui.schedule(function () self:updateState()end ,OxFlowersNewDefine.TIME_INTERVAL,OxFlowersNewDefine.TIME_USER_ADD_SCORE)
    end
...
function OxNew:OnSubAddScore(evt)
    ...
    if ViewID == OxFlowersNewDefine.MYSELF_VIEW_ID and self:IsNotLookonMode() then
        self.addSourceView:hide()
    end
    ...
    if self.cbPlayStatus[p.wAddScoreUser]==OxFlowersNewDefine.USEX_PLAYING then
        self:OnUserAddScore(wViewChairID,p.lAddScoreCount,false)
    end
    oxui.playSound("ADD_SCORE" ,false, OxFlowersNewDefine.wav)
end
            ...
            if v then
                ...
                for i=1, OxFlowersNewDefine.GAME_PLAYER do
                    if not self.m_GameUser.sixHeros[i-1] then
                        ...
                        self.cbPlayStatus[i-1] = OxFlowersNewDefine.USEX_NULL
                    else
                        self.wViewChairID[i-1] = self.m_GameKernel:SwitchViewChairID(i-1)
                        self.cbPlayStatus[i-1]= OxFlowersNewDefine.USEX_PLAYING
...
--
--        if p.bChange then
--            for i=1,OxFlowersNewDefine.MAXCOUNT do
...
--                    self.handCardControl[OxFlowersNewDefine.MYSELF_VIEW_ID]:OnChangeCard(bOldCardData,bChangeData)
--                end
--            end
--        end
--        self.changeView:hide()
...
--        self.gameState = OxFlowersNewDefine.SUB_S_CHANGE_CARD
--        self:stopAllTimer()
--        self.timerCount = OxFlowersNewDefine.TIME_USER_CHANGE_CARD
--        self.t_Change = oxui.schedule(function()
--            self:updateState()
--        end ,
--        OxFlowersNewDefine.TIME_INTERVAL,
--        OxFlowersNewDefine.TIME_USER_CHANGE_CARD)
--    end
end
--摊牌
function OxNew:OnSubOpenCard(evt)
    ...
    if not self:IsNotLookonMode() then
        self.handCardControl[OxFlowersNewDefine.MYSELF_VIEW_ID]:SetPositively(true)
    end
    ...
    if self.cbHandCardData[wChairID+1] and self.cbHandCardData[wChairID+1][1] >0 then
        ...
        local bCardType = GameLogic:GetCardType2(self.cbHandCardData[wChairID+1],OxFlowersNewDefine.MAXCOUNT)
        print("bCardType=" .. bCardType)
        if bCardType >= OxFlowersNewDefine.OX_THREE_SAME then
    ...
    if(self.cbPlayStatus[wChairID]==OxFlowersNewDefine.USEX_PLAYING) then
        self.openView:show()
    end
...
function OxNew:OnSubEndOpen(evt)
    ...
    if self:IsCurrentUser(wID) then
        ...
        oxui.playSound("OPEN_CARD",false,OxFlowersNewDefine.wav)
    end
    ...
    if not self.isopen and wID == self:GetMeChairID() then
        ...
        dump(cardData)
        local bCardValue = GameLogic:GetCardType2(cardData,OxFlowersNewDefine.MAXCOUNT,cardData)
        if bCardValue == OxFlowersNewDefine.OX_THREE_SAME then
            bCardValue = 13 -- 葫芦
        end
        if bCardValue == OxFlowersNewDefine.OX_FOUR_SAME then
            bCardValue = 14 --炸弹
        end
        if bCardValue == OxFlowersNewDefine.OX_FOURKING then
            bCardValue = 15 -- 五小牛
        end

        if bCardValue > 0 then
            self:SetAnimCard(OxFlowersNewDefine.MYSELF_VIEW_ID,bCardValue)
...
function OxNew:SetAnimCard(wViewChairID,number)
    ...
    if not self.handCardControl[wViewChairID]:getChildByName("AnimationOxType") then
        ...
        local posx
        if wViewChairID == OxFlowersNewDefine.MYSELF_VIEW_ID then
    ...
    if number then
        oxui.playSound("ox" .. number,false,OxFlowersNewDefine.m4a)
    end

    if number == 0 and wViewChairID == OxFlowersNewDefine.MYSELF_VIEW_ID then
        self.handCardControl[OxFlowersNewDefine.MYSELF_VIEW_ID]:setUnSelected()
    end
...
function OxNew:onCleanup()
    ...
    display.removeSpriteFramesWithFile("oxflowersnew/UIGameIng.plist","oxflowersnew/UIGameIng.png")
    display.removeSpriteFramesWithFile("oxflowersnew/talkWidgetPlist.plist","oxflowersnew/talkWidgetPlist.png")
    display.removeSpriteFramesWithFile("oxflowersnew/settingWidgetPlist.plist","oxflowersnew/settingWidgetPlist.png")
    display.removeSpriteFrameByImageName("oxflowersnew/u_game_table.jpg") 
    display.removeSpriteFrameByImageName("oxflowersnew/bg_oxnew_logo.png") 
...
function OxNew:RegistEventManager()
    ...
    eventListeners[OxFlowersNewDefine.GAME_SCENCE] = handler(self, self.OnGameScenceMsg)-- 场景的消息
    eventListeners[OxFlowersNewDefine.GAME_START] = handler(self, self.OnSubGameStart)  -- 游戏开始
    eventListeners[OxFlowersNewDefine.GAME_ADD_SCORE] = handler(self, self.OnSubAddScore)-- 加注结果
    eventListeners[OxFlowersNewDefine.GAME_PLAYER_EXIT] = handler(self, self.OnSubPlayerExit)-- 玩家退出
    eventListeners[OxFlowersNewDefine.GAME_SEND_CARD] = handler(self, self.OnSubSendCard) -- 发牌消息
    eventListeners[OxFlowersNewDefine.GAME_CALL_BANKER] = handler(self, self.OnSubCallBanker)--用户叫庄
    eventListeners[OxFlowersNewDefine.GAME_ALL_CARD] = handler(self, self.OnSubAllCard)--发送基数
    eventListeners[OxFlowersNewDefine.GAME_AMDIN_COMMAND] = handler(self, self.OnSubChangeCard)--系统控制
    eventListeners[OxFlowersNewDefine.GAME_BANKER_OPERATE] = handler(self, self.OnSubOpenCard)--存取款
    eventListeners[OxFlowersNewDefine.GAME_OPEN_CARD] = handler(self, self.OnSubEndOpen)
    eventListeners[OxFlowersNewDefine.GAME_PLAYER_OPEN] = handler(self, self.OnSubPlayerOpen)

    --eventListeners["GF_USER_CHAT"] = handler(self, self.ChatCallBack)
    eventListeners[OxFlowersNewDefine.GAME_OVER] = handler(self, self.OnSubGameOver) --结束
...
function OxNew:updateState()
    ...
    if  self.timerCount <= 0 then
        if self.gameState == OxFlowersNewDefine.GS_TK_FREE and self:getMyStatus() ~= US_READY then
            self.m_GameUser:LeaveType()
        end 

        if self.gameState == OxFlowersNewDefine.SUB_S_CHANGE_OPEN then
            self:sendOpenMsg()
        end
        self:stopAllTimer()
        return
    ...
    if self.gameState == OxFlowersNewDefine.GS_TK_FREE then
        self.lab:setString("等待玩家准备:" ..  self.timerCount)
    elseif self.gameState == OxFlowersNewDefine.SUB_S_CALL_BANKER then
        self.lab:setString("叫庄:" ..  self.timerCount)
    elseif self.gameState == OxFlowersNewDefine.SUB_S_ADD_SCORE then
        self.lab:setString("等待他人加注:" ..  self.timerCount)
    elseif self.gameState == OxFlowersNewDefine.SUB_S_CALL_OTH then
        self.lab:setString("等待他人叫庄:" ..  self.timerCount) 
    elseif self.gameState == OxFlowersNewDefine.SUB_S_OPEN_CARD then
        self.lab:setString("等待他人开牌:" ..  self.timerCount)
    elseif self.gameState == OxFlowersNewDefine.SUB_S_USER_OPEN then
...
function OxNew:setUserPlayingStatus(wChairID,statu)
    ...
    if wChairID <= OxFlowersNewDefine.GAME_PLAYER then
...
function OxNew:fntCardData(para)
    ...
    for key, var in ipairs(data) do
        ...
        if key % OxFlowersNewDefine.MAXCOUNT == 0 then
            ix = ix + 1
            iy = 1
        else
            iy = iy + 1
...
function OxNew:moveDisPachCard()
    ...
    local index = 0
    for j=winUser ,winUser + OxFlowersNewDefine.GAME_PLAYER -1 do
        local w= j%OxFlowersNewDefine.GAME_PLAYER
        local wViewChairID = self.wViewChairID[w]
        if self.cbPlayStatus[w] == OxFlowersNewDefine.USEX_PLAYING then
            for i=1,OxFlowersNewDefine.MAXCOUNT do
                local poker
                if wViewChairID == OxFlowersNewDefine.MYSELF_VIEW_ID then
                    print("ww.. " .. w)
...
function OxNew:OnSendCardFinish()
    ...
    if self:IsNotLookonMode()then
        self.openView:show()
        self.handCardControl[OxFlowersNewDefine.MYSELF_VIEW_ID]:SetShow(true)
    end
 ---[[

  if self.cbHandCardData[wMeChairID+1] and self.cbHandCardData[wMeChairID+1][1] >0 then
        --dump(self.cbHandCardData[wChairID+1] )
        local bCardType = GameLogic:GetCardType2(self.cbHandCardData[wMeChairID+1],OxFlowersNewDefine.MAXCOUNT)
        print("无花牛 -- bCardType=" .. bCardType)
        if bCardType >= OxFlowersNewDefine.OX_THREE_SAME then
            ...
            --保存牛信息
            --self.bUserOxCard[wMeChairID]=true
        end
    end
 --]]
    if not self.t_Open and self:IsNotLookonMode() then 
        self.gameState = OxFlowersNewDefine.SUB_S_OPEN_CARD
        self.timerCount = OxFlowersNewDefine.TIME_USER_OPEN_CARD
        self.t_Open = oxui.schedule(function()
            self:updateState()
        end ,
        OxFlowersNewDefine.TIME_INTERVAL,
        OxFlowersNewDefine.TIME_USER_OPEN_CARD)
    end
...
function OxNew:onOxEnable(isShoot,wchairid)
    ...
    --    print("wchairid =="..wchairid)
    --    dump(cbCardData)
    local newData = GameLogic:sortCardRes(cbCardData,OxFlowersNewDefine.MAXCOUNT)
...
--
--    if t == OxFlowersNewDefine.animHead then
