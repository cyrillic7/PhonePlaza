...
local userPos = {
    cc.p(display.cx -130 ,display.cy + 210),--1号位置 对家 
    cc.p(display.cx + 190,display.cy+50),--2号位置 对家
    cc.p(90,190),--4号位置 自己
    cc.p(display.cx - 420,display.cy+50),--5号位置 对家 
}
...
function Player:ctor(Json,Kernel)
    ...
    for var=1, OxFlowersNewDefine.GAME_PLAYER do
...
--    dump(evt.para)
    ...
    --自己
    print("evt.para.lScore = " .. evt.para.lScore)
    
    if ViewID==OxFlowersNewDefine.MYSELF_VIEW_ID then
        --头像框-自己
        ...
        self.player3:show()
        --头像-自己
        self.head3:setSpriteFrame("pic/face/"..evt.para.wFaceID..".png")
        self.head3:show()
        self.head3:setScale(0.45)

        --昵称-自己
        self.name3:setString(G_TruncationString(evt.para.szNickName,15))
        self.name3:show() 
        --分数-自己
        print("evt.para.lScore = " .. evt.para.lScore)
        self.gold3:setString(evt.para.lScore)
        self.gold3:show()
...
function Player:setCellScore(wChairID,score)
    ...
    if wChairID == OxFlowersNewDefine.MYSELF_VIEW_ID then 
...
function Player:setSouceTexture(vid,iszheng)
    ...
    self["TXT" .. vid]:setProperty("0", "oxflowersnew/Font/" .. str ..  ".png", 23, 31, "0")
