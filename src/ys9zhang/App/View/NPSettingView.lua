...
local  NPSettingView = class("NPSettingView",function()
...
function NPSettingView:loadUI()
	self.mainWidget = GameUtil:widgetFromCocostudioFile("ys9zhang/settingWidget")
...
function NPSettingView:onSoundVolumeChange(pSender,eventtype)
	if eventtype == 0 then
...
function NPSettingView:onMusicVolumeChange(pSender,eventtype)
	if eventtype == 0 then
		local  percent = pSender:getPercent()
		local   volume= percent/100.0
...
function NPSettingView:onClickOk(pSender,touchType)
