...
function GameLogic:GetCardType(cbCardData,cbCardCount,bcOutCadData1)

    if cbCardCount ~= 5 then
        return 0
    end
    local cbCardDataSort = cbCardData
    local bcOutCadData =bcOutCadData1 or {} 
    local iSingleA = {}
    local iGetTenCount = 0
    local iBigValue = 0
    local bcMakeMax = {}
    self:SortCardList(cbCardDataSort,cbCardCount) 
    print("==================") 
    ...
    if iGetTenCount>=3 then
        ...
        if(self:GetCardColor(cbCardDataSort[1]) == aceType) then
            ...
            if iBigValue % 10 == 0 then
                ...
                if table.getn(bcOutCadData) > 0 then
                    return iBigValue%10,bcOutCadData
                else
                    return iBigValue%10,cbCardDataSort
                end
            end
        end
    end
    if iGetTenCount==2 or (iGetTenCount==1 and self:GetCardColor(cbCardDataSort[1]) == aceType) then
        ...
        if(self:GetCardColor(cbCardDataSort[1]) ==aceType and self:GetCardColor(cbCardDataSort[2]) == aceType) then
            ...
            if self:GetCardColor(cbCardDataSort[1])==aceType then
                ...
                if((self:GetCardLogicValue(cbCardDataSort[3])+self:GetCardLogicValue(cbCardDataSort[4])+self:GetCardLogicValue(cbCardDataSort[5]))%10==0) then
                    if bcOutCadData then
            ...
            for j = 2,cbCardCount do
                if j ~= n then
                    if(self:GetCardLogicValue(cbCardDataSort[n])+self:GetCardLogicValue(cbCardDataSort[j]))%10==0 then
                        ...
                        --dump(iSingleA)
                        --print("iBigValue1=" .. ((self:GetCardLogicValue(iSingleA[1])+self:GetCardLogicValue(iSingleA[2]))%10))

                        --print("iBigValue2=" .. iBigValue)
...
function GameLogic:GetTimes(cbCardData, cbCardCount)
    ...
    if  cbCardCount~=RedninebattleDefine.MAXCOUNT then
        return 0
    end
...
function GameLogic:SortCardList(cbCardData , cbCardCount)
    ...
    for i=1,cbCardCount do
        cbSortValue[i]=self:GetCardNewValue(cbCardData[i])
    end
...
--    dump(cbCardData)
--    for var=1, 5 do
--        print("datavalue= " ..  GameLogic:GetCardLogicValue(cbCardData[var]))
--    end
    --table.sort(cbSortValue,function(a,b) return a > b end)
    return cbCardData
end
...
function GameLogic:CompareCard(cbFirstData,cbNextData,cbCardCount, FirstOX,NextOX)
    ...
    if FirstOX then
        --获取点数
        local cbNextType,outFirstData=self:GetCardType(cbNextData,cbCardCount)
        local cbFirstType,outNextData=self:GetCardType(cbFirstData,cbCardCount)
