...
oxui.playSound = function(name,isLoop,t)
    ...
    local type = t
    local str = ".mp3" 
    if type == RedninebattleDefine.wav then
        type = ".wav"
    elseif type == RedninebattleDefine.m4a then
        type = ".m4a"
    else
        type = ".mp3"
    end
    local n = "redninebattle/ogg/" .. name .. type
    
    SoundManager:playMusicEffect(n,loop,false)    
end
 
oxui.cacheAnimationList = oxui.cacheAnimationList or {}
oxui.animPath = "redninebattle/animation/"
...
oxui.removeAll = function ()  
    ...
    --cc.SpriteFrameCache:getInstance():removeSpriteFramesFromTexture("redninebattle/u_game_table.jpg")
