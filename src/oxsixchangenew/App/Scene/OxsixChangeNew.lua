...
--声明游戏场景类
local OxsixChangeNew  = class("OxsixChangeNew", function() return display.newScene("OxsixChangeNew")end)
...
local CMD_C_OxCard = {}
--本类的构造函数
function OxsixChangeNew:ctor(args)
    display.addSpriteFrames("oxsixchangenew/UIGameIng.plist","oxsixchangenew/UIGameIng.png")
    display.addSpriteFrames("oxsixchangenew/talkWidgetPlist.plist","oxsixchangenew/talkWidgetPlist.png")
    display.addSpriteFrames("oxsixchangenew/settingWidgetPlist.plist","oxsixchangenew/settingWidgetPlist.png")
    self.App=AppBaseInstanse.OxsixChangeNewApp
    self:setNodeEventEnabled(true)
    self.gameState = OxsixChangeDefine.SUB_S_GAME_START
    ...
    local gameAttribute =
        {
            wKindID=OxsixChangeDefine.KIND_ID,
            wPlayerCount=OxsixChangeDefine.GAME_PLAYER,
            dwClientVersion=OxsixChangeDefine.VERSION_CLIENT,
            pszGameName=OxsixChangeDefine.GAME_NAME,
        }
...
--初始化数据
function OxsixChangeNew:FreeAllData()
    --首发牌用户
    self.HandCardCount={0,0}
    for i=1, OxsixChangeDefine.GAME_PLAYER do
...
--加载资源
function OxsixChangeNew:InitUnits()

    --读取json 文件
    local node = oxui.getUIByName("oxsixchangenew/oxsixLayer.json")
    self.node  = node
    local changeView = oxui.getUIByName("oxsixchangenew/changeView.json")
    changeView:addTo(self,10000)
    changeView:setPosition(display.cx-100,display.cy-100)
    node:addTo(self)
    ...
    self.changeTouchView = oxui.getNodeByName(changeView,"changeTouchView")

    self.light = oxui.getNodeByName(self.changeTouchView,"light")
    self.changeBg = oxui.getNodeByName(self.changeTouchView,"changeBg")
    self.charge = oxui.getNodeByName(self.changeTouchView,"charge")
...
--初始化牌堆
function OxsixChangeNew:InitHeapCard()
    --发牌张数
...
--初始化手牌
function OxsixChangeNew:InitHandCardControl()
    for i=1, OxsixChangeDefine.GAME_PLAYER do
        ...
        if i == OxsixChangeDefine.MYSELF_VIEW_ID then
...
--开始发牌
function OxsixChangeNew:StartDisPachCard()
    --发牌张数
    self:stopAllTimer()
...
function OxsixChangeNew:DisPatchOneCardEnd(evt)
    oxui.playSound("SEND_CARD",false,OxsixChangeDefine.wav)
...
--控件消息
function OxsixChangeNew:ButtonMsg()
    -------------------------------------------------
    --开始按钮事件
    -------------------------------------------------
    ...
    self.btnBanker:onButtonClicked(function ()
        ...
        self:send(OxsixChangeDefine.SUB_C_CALL_BANKER,CMD_C_CallBanker ,"CMD_C_CallBanker")
        self.BankerView:hide()
        self.clock:hide()
    end)
    ...
    self.btnChange:onButtonPressed(function ()
        self:playScaleAnimation(true,self.btnChange)
    end)

    --设置按钮抬起
    self.btnChange:onButtonRelease(function ()
        self:playScaleAnimation(false,self.btnChange)
    end)

    self.btnChangeCancel:onButtonPressed(function ()
        self:playScaleAnimation(true,self.btnChangeCancel)
    end)

    --设置按钮抬起
    self.btnChangeCancel:onButtonRelease(function ()
        self:playScaleAnimation(false,self.btnChangeCancel)
    end)

    self.btnChange:onButtonClicked(function ()
        local bMyShootCard = self.handCardControl[OxsixChangeDefine.MYSELF_VIEW_ID]:getShootCard()
        if bMyShootCard then
            if self.isChange then
                self:addAnimChangeView(OxsixChangeDefine.animHead,true)
                self.changeTouchView:show()
                self.isChange = false
            end
        else
            self.gameState = OxsixChangeDefine.SUB_S_CALL_unChange
            print("选择你要弹起的牌")
        end
    end)

    self.btnChangeCancel:onButtonClicked(function ()
       -- if self.isChange then
            self:sendChangeMsgNo()
            self.changeView:hide()
       -- end
    end)
    ...
    self.btnTip:onButtonClicked(function ()
        ...
        local bCardValue = GameLogic:GetCardType2(cardData,OxsixChangeDefine.MAXCOUNT,cardData)
        if bCardValue > 0 then
            self.handCardControl[OxsixChangeDefine.MYSELF_VIEW_ID]:SetShootOXOpoker()
        else
            self.openView:hide()
            self:SetAnimCard(OxsixChangeDefine.MYSELF_VIEW_ID,0)
            local bOX = 0
            CMD_C_OxCard = {bOX = bOX}
            self:send(OxsixChangeDefine.SUB_C_OPEN_CARD,CMD_C_OxCard ,"CMD_C_OxCard")
        end
    end)

    self.changeTouchView:addNodeEventListener(cc.NODE_TOUCH_EVENT, function (e)
        if e.name == "began" then
            local bMyShootCard = self.handCardControl[OxsixChangeDefine.MYSELF_VIEW_ID]:getShootCard()
            if bMyShootCard then
                CMD_C_CHANGE_CARD = {bChange = 1,wPlayer =self:GetMeChairID() ,cbChangeCard = bMyShootCard}
                self:send(OxsixChangeDefine.SUB_C_CHANGE_CARD,CMD_C_CHANGE_CARD ,"CMD_C_ChangeCard")
                self.changeView:hide()
                self.changeTouchView:hide()
            else
                self.gameState = OxsixChangeDefine.SUB_S_CALL_unChange
                print("请选择你要弹起的牌")
            end
...
function OxsixChangeNew:sendChangeMsgNo()
    CMD_C_CHANGE_CARD = {bChange = 0,wPlayer =self:GetMeChairID(),cbChangeCard = 0}
    self:send(OxsixChangeDefine.SUB_C_CHANGE_CARD,CMD_C_CHANGE_CARD ,"CMD_C_ChangeCard")
    self.changeView:hide()
end

function OxsixChangeNew:sendOpenMsg()
    local cardData = self.cbHandCardData[self:GetMeChairID()+1]
    local bOX = 0
    if GameLogic:GetOxCard(cardData,OxsixChangeDefine.MAXCOUNT) then
        bOX = 1
    else
        self.openView:hide()
    end
...
function OxsixChangeNew:sendAddSource(index)
    print("addSouceindex" .. index)
    dump(self.lUserMaxScore)
    CMD_C_AddScore.lScore = self.lUserMaxScore[index]
    self:send(OxsixChangeDefine.SUB_C_ADD_SCORE,CMD_C_AddScore ,"CMD_C_AddScore")
end
...
---------------[[以下是接收游戏服务端消息处理]]--------------
--场景
function OxsixChangeNew:OnGameScenceMsg(evt)
    print("场景====++++++++++++++++++++++++++++++++++")
    dump(evt.para)
    ...
    if gameStatus == OxsixChangeDefine.GS_TK_FREE then
        print("空闲状态")
        ...
        if self:getMyStatus() == US_SIT and self:getMyStatus() ~= US_PLAYING then
            ...
            if not self.t_Start then
                self.gameState = OxsixChangeDefine.GS_TK_FREE
                self.timerCount = 10
                self.t_Start = oxui.schedule(function()self:updateState() end,OxsixChangeDefine.TIME_INTERVAL,OxsixChangeDefine.TIME_USER_START_GAME)
            end

            self.btnStart:show() 
        end
        --叫庄状态
    elseif gameStatus == OxsixChangeDefine.GS_TK_CALL then
        ...
        --更新状态
        for i=0,OxsixChangeDefine.GAME_PLAYER - 1 do
        ...
        if m_wBankerUser == self:GetMeChairID() then
            if not self.t_Bander then
                self:stopAllTimer()
                self.gameState = OxsixChangeDefine.SUB_S_CALL_BANKER
                self.timerCount = OxsixChangeDefine.TIME_USER_CALL_BANKER
                self.t_Bander = oxui.schedule(function()
                    self:updateState()
                end ,
                OxsixChangeDefine.TIME_INTERVAL,
                OxsixChangeDefine.TIME_USER_CALL_BANKER)
            end
        else
            if not self.t_Bander then
                self:stopAllTimer()
                self.gameState = OxsixChangeDefine.SUB_S_CALL_OTH
                self.timerCount = OxsixChangeDefine.TIME_USER_CALL_BANKER
                self.t_Bander = oxui.schedule(function()
                    self:updateState()
                end ,
        ...
        --加注状态
    elseif gameStatus == OxsixChangeDefine.GS_TK_SCORE then
        ...
        self.gameState = OxsixChangeDefine.SUB_S_ADD_SCORE
        if not self.t_Change then
            self:stopAllTimer()
            self.timerCount = OxsixChangeDefine.TIME_USER_ADD_SCORE
            self.t_Change = oxui.schedule(function()
                self:updateState()
            end ,
            OxsixChangeDefine.TIME_INTERVAL,
            OxsixChangeDefine.TIME_USER_ADD_SCORE)
        end
        self.wBankerUser = m_wBankerUser
        dump(self.cbPlayStatus)

        --加注状态
    elseif gameStatus == OxsixChangeDefine.GS_TK_PLAYING then
...
--        self.gameState = OxsixChangeDefine.TIME_USER_OPEN_ING
--        if not self.t_Open then
--            self:stopAllTimer()
--            self.timerCount = OxsixChangeDefine.TIME_USER_OPEN_ING
--            self.t_Open = oxui.schedule(function()
--                self:updateState()
--            end ,
--            OxsixChangeDefine.TIME_INTERVAL,
--            OxsixChangeDefine.TIME_USER_OPEN_ING)
--        end
    end
...
function OxsixChangeNew:OnSubPlayerOpen(evt)
    print("所有玩家都开完牌")
    self:stopAllTimer()
    ...
    for i=OxsixChangeDefine.GAME_PLAYER + banker - 1,banker ,-1 do  
        local w = i%OxsixChangeDefine.GAME_PLAYER 
        if self.cbPlayStatus[w]==OxsixChangeDefine.USEX_PLAYING and w ~= self:GetMeChairID() then
            local wViewChairID =  self.wViewChairID[ w] 
            if self.cbHandCardData[w+1] then
                self.handCardControl[wViewChairID]:SetCardData(self.cbHandCardData[w+1],OxsixChangeDefine.MAXCOUNT)
                local time = cc.DelayTime:create(t)
                t = t+1
                index = index + 1
                local fuc = cc.CallFunc:create(function()
                    self.handCardControl[wViewChairID]:SetShow(true)
                    local num = GameLogic:GetCardType2(self.cbHandCardData[w+1],OxsixChangeDefine.MAXCOUNT)
                    if num > 0 then
    ...
    local c = cc.CallFunc:create(function()
        self:stopAllTimer()
        self:send(OxsixChangeDefine.SUB_C_OPEN_END)
    end )
    self:runAction(cc.Sequence:create(d,c))
end
--游戏结束
function OxsixChangeNew:OnSubGameOver(evt)
    ...
    function callFuck()
        ...
        self.gameState = OxsixChangeDefine.GS_TK_FREE
        if not self.t_Start then
            self.timerCount = OxsixChangeDefine.TIME_USER_START_GAME
            self.t_Start = oxui.schedule(function(t) self:updateState(OxsixChangeDefine.TIME_USER_START_GAME - t) end ,OxsixChangeDefine.TIME_INTERVAL,OxsixChangeDefine.TIME_USER_START_GAME)
        end
    ...
    if self:IsNotLookonMode() then
        ...
        if pGameEnd.lGameScore[self:GetMeChairID()+1] > 0 then
            oxui.playSound("GAME_WIN",false,OxsixChangeDefine.m4a)
            ...
            oxui.playSound("GAME_LOST",false,OxsixChangeDefine.m4a)
    ...
    for  i=1,OxsixChangeDefine.GAME_PLAYER do
        ...
        if self.cbPlayStatus[i-1]==OxsixChangeDefine.USEX_PLAYING then
            if souce >= 0 then
...
function OxsixChangeNew:stopAllTimer()
    self.clock:hide()
...
--游戏开始
function OxsixChangeNew:OnSubGameStart(evt)
    print("游戏开始")
    local user = evt.para
    --dump(user)
    ...
    self.charge:setString("" .. m_lScoreTax)


    if self:IsNotLookonMode() and  self.lTurnMaxScore > 0 and user.wBankerUser ~= self:GetMeChairID() and self.cbPlayStatus[self.wBankerUser] == OxsixChangeDefine.USEX_PLAYING then
        self.bWaitAdd=true
    ...
    oxui.playSound("GAME_START",false,OxsixChangeDefine.m4a)
    ...
    if not self.t_AddSource then
        ...
        print("加分")
        self.timerCount = OxsixChangeDefine.TIME_USER_ADD_SCORE
        self.t_AddSource = oxui.schedule(function () self:updateState()end ,OxsixChangeDefine.TIME_INTERVAL,OxsixChangeDefine.TIME_USER_ADD_SCORE)
    end
...
--加注结果
function OxsixChangeNew:OnSubAddScore(evt)
    print("加注结果11111")
    local p = evt.para -- lAddScoreCount --加注数目
    local ViewID=self.m_GameKernel:SwitchViewChairID(p.wAddScoreUser)
    print("ViewID 。。 " .. ViewID)
    if ViewID == OxsixChangeDefine.MYSELF_VIEW_ID and self:IsNotLookonMode() then
        self.addSourceView:hide()
        print("加注时器")
    end
    ...
    if self.cbPlayStatus[p.wAddScoreUser]==OxsixChangeDefine.USEX_PLAYING then
        self:OnUserAddScore(wViewChairID,p.lAddScoreCount,false)
    end
    print("加注数目"  .. p.lAddScoreCount )
    oxui.playSound("ADD_SCORE" ,false, OxsixChangeDefine.wav)
end

function OxsixChangeNew:OnUserAddScore(id)
--移动筹码
--m_GoldMove.SetAddScore(wChairID,lTableScore,m_JetonStartPos[wChairID]);
end
--加注显示
function OxsixChangeNew:updateAddScore(scrs)

end
--用户强退
function OxsixChangeNew:OnSubPlayerExit(evt)
    print("用户强退1111")
    --dump(evt.para);
end
--发牌消息
function OxsixChangeNew:OnSubSendCard(evt)
    print("发牌消息111")
...
--用户叫庄
function OxsixChangeNew:OnSubCallBanker(evt)
    local p = evt.para
            ...
            if v then
                ...
                for i=1, OxsixChangeDefine.GAME_PLAYER do
                    if not self.m_GameUser.sixHeros[i-1] then
                        ...
                        self.cbPlayStatus[i-1] = OxsixChangeDefine.USEX_NULL
                    else
                        self.wViewChairID[i-1] = self.m_GameKernel:SwitchViewChairID(i-1)
                        self.cbPlayStatus[i-1]= OxsixChangeDefine.USEX_PLAYING
...
--发送基数
function OxsixChangeNew:OnSubGameBase(evt)
    print("发送基数111")
    --dump(evt.para);
end
--用户换牌
function OxsixChangeNew:OnSubChangeCard(evt)
    print("用户换牌11")
    --bChange;                            //换牌标志
    --wPlayerID;                         //换牌用户
    --cbCardData;                        //换牌数据
    ...
    if ViewID == OxsixChangeDefine.MYSELF_VIEW_ID and self:IsNotLookonMode() then
        ...
        if p.bChange then
            for i=1,OxsixChangeDefine.MAXCOUNT do
                if (self.cbHandCardData[wChangeID][i]==bOldCardData) then
                    ...
                    self.handCardControl[OxsixChangeDefine.MYSELF_VIEW_ID]:OnChangeCard(bOldCardData,bChangeData)
                end
            end
        end
        self.changeView:hide()
    ...
    if not self.t_Change and self:IsNotLookonMode() then
        self.gameState = OxsixChangeDefine.SUB_S_CHANGE_CARD
        self:stopAllTimer()
        self.timerCount = OxsixChangeDefine.TIME_USER_CHANGE_CARD
        self.t_Change = oxui.schedule(function()
            self:updateState()
        end ,
        OxsixChangeDefine.TIME_INTERVAL,
        OxsixChangeDefine.TIME_USER_CHANGE_CARD)
    end
end
--摊牌
function OxsixChangeNew:OnSubOpenCard(evt)
    print("所有人换牌完成 摊牌")
    local wChairID= self:GetMeChairID()

    if not self:IsNotLookonMode() then
        self.handCardControl[OxsixChangeDefine.MYSELF_VIEW_ID]:SetPositively(true)
    end
    local wViewChairID = self.wViewChairID[wChairID]
    self.changeView:hide()
    if not self.t_Open then
        self:stopAllTimer()
        self.gameState = OxsixChangeDefine.SUB_S_CHANGE_OPEN
        self.timerCount = OxsixChangeDefine.TIME_USER_OPEN_CARD
        self.t_Open = oxui.schedule(function()
            self:updateState()
        end ,
        OxsixChangeDefine.TIME_INTERVAL,
        OxsixChangeDefine.TIME_USER_OPEN_CARD)
    end
    ...
    if self.cbHandCardData[wChairID+1] and self.cbHandCardData[wChairID+1][1] >0 then
        ...
        local bCardType = GameLogic:GetCardType2(self.cbHandCardData[wChairID+1],OxsixChangeDefine.MAXCOUNT)
        print("bCardType=" .. bCardType)
        if bCardType >= OxsixChangeDefine.OX_THREE_SAME then
    ...
    if(self.cbPlayStatus[wChairID]==OxsixChangeDefine.USEX_PLAYING) then
        self.openView:show()
    end

end

function OxsixChangeNew:OnSubEndOpen(evt)
    print("开牌")
    local p = evt.para
    --dump(evt)
    local wChairID = self:GetMeChairID()
    ...
    if self:IsCurrentUser(wID) then
        ...
        oxui.playSound("OPEN_CARD",false,OxsixChangeDefine.wav)
    end
    ...
    if not self.isopen and wID == self:GetMeChairID() then
        ...
        local bCardValue = GameLogic:GetCardType2(cardData,OxsixChangeDefine.MAXCOUNT,cardData)
        if bCardValue > 0 then
            self:SetAnimCard(OxsixChangeDefine.MYSELF_VIEW_ID,bCardValue)
...
function OxsixChangeNew:SetAnimCard(wViewChairID,number)
    ...
    if not self.handCardControl[wViewChairID]:getChildByName("AnimationOxType") then
        ...
        local posx
        if wViewChairID == OxsixChangeDefine.MYSELF_VIEW_ID then
    ...
    if number then
        oxui.playSound("ox" .. number,false,OxsixChangeDefine.m4a)
    end

    if number == 0 and wViewChairID == OxsixChangeDefine.MYSELF_VIEW_ID then
        self.handCardControl[OxsixChangeDefine.MYSELF_VIEW_ID]:setUnSelected()
    end


end
--场景进入
function OxsixChangeNew:onEnter()
...
--    dump(AppBaseInstanse.PLAZACENTER_APP.Message.LS_GetHornMessage)
...
--        para = {szNickName="牛逼奥巴马",szLabaText="超级杯19:35视频播恒大-苏宁 冬季转会一览2"}
--    })
end
--场景销毁
function OxsixChangeNew:onExit()
    print("退出了") 
    SoundManager:stopMusicBackground()
    self.ClientKernel:removeListenersByTable(self.GameeventHandles) 
end
function OxsixChangeNew:onCleanup()
    print("场景销毁") 
    ...
    display.removeSpriteFramesWithFile("oxsixchangenew/UIGameIng.plist","oxsixchangenew/UIGameIng.png")
    display.removeSpriteFramesWithFile("oxsixchangenew/talkWidgetPlist.plist","oxsixchangenew/talkWidgetPlist.png")
    display.removeSpriteFramesWithFile("oxsixchangenew/settingWidgetPlist.plist","oxsixchangenew/settingWidgetPlist.png")
    display.removeSpriteFrameByImageName("oxsixchangenew/u_game_table.jpg") 
    display.removeSpriteFrameByImageName("oxsixchangenew/bg_sixchange_logo.png") 
    self.m_GameUser:OnFreeInterface()
    self.m_GameKernel:OnFreeInterface()
    self.ClientKernel:cleanup()
end
--事件管理
function OxsixChangeNew:RegistEventManager()

    --游戏类操作消息
    local eventListeners = eventListeners or {}
    eventListeners[OxsixChangeDefine.GAME_SCENCE] = handler(self, self.OnGameScenceMsg)-- 场景的消息
    eventListeners[OxsixChangeDefine.GAME_START] = handler(self, self.OnSubGameStart)  -- 游戏开始
    eventListeners[OxsixChangeDefine.GAME_ADD_SCORE] = handler(self, self.OnSubAddScore)-- 加注结果
    eventListeners[OxsixChangeDefine.GAME_PLAYER_EXIT] = handler(self, self.OnSubPlayerExit)-- 玩家退出
    eventListeners[OxsixChangeDefine.GAME_SEND_CARD] = handler(self, self.OnSubSendCard) -- 发牌消息
    eventListeners[OxsixChangeDefine.GAME_CALL_BANKER] = handler(self, self.OnSubCallBanker)--用户叫庄
    eventListeners[OxsixChangeDefine.GAME_BASE] = handler(self, self.OnSubGameBase)--发送基数
    eventListeners[OxsixChangeDefine.GAME_CHANGE_CARD] = handler(self, self.OnSubChangeCard)--换牌
    eventListeners[OxsixChangeDefine.GAME_CHANGE_OPEN] = handler(self, self.OnSubOpenCard)--开牌
    eventListeners[OxsixChangeDefine.GAME_OPEN_CARD] = handler(self, self.OnSubEndOpen)
    eventListeners[OxsixChangeDefine.GAME_PLAYER_OPEN] = handler(self, self.OnSubPlayerOpen)

    --eventListeners["GF_USER_CHAT"] = handler(self, self.ChatCallBack)
    eventListeners[OxsixChangeDefine.GAME_OVER] = handler(self, self.OnSubGameOver) --结束
    self.GameeventHandles = self.ClientKernel:addEventListenersByTable( eventListeners )
end

--function OxsixChangeNew:ChatCallBack(evt)
--    dump(evt)
--end

--定义按钮缩放动画函数
function OxsixChangeNew:playScaleAnimation(less, pSender)
    local  scale = less and 0.9 or 1
    pSender:runAction(cc.ScaleTo:create(0.2,scale))
end

function OxsixChangeNew:updateState()
    self.timerCount = self.timerCount - 1
    --self.lbltime:setString(self.timerCount)
    self.clock:show()
    if  self.timerCount <= 0 then
        if self.gameState == OxsixChangeDefine.GS_TK_FREE and self:getMyStatus() ~= US_READY then
            self.m_GameUser:LeaveType()
        end

        if self.gameState == OxsixChangeDefine.SUB_S_CHANGE_CARD then
            self:sendChangeMsgNo()
        end

        if self.gameState == OxsixChangeDefine.SUB_S_CHANGE_OPEN then
            self:sendOpenMsg()
        end
        self:stopAllTimer()
        return
    ...
    if self.gameState == OxsixChangeDefine.GS_TK_FREE then
        self.lab:setString("等待玩家准备:" ..  self.timerCount)
    elseif self.gameState == OxsixChangeDefine.SUB_S_CALL_BANKER then
        self.lab:setString("叫庄:" ..  self.timerCount)
    elseif self.gameState == OxsixChangeDefine.SUB_S_ADD_SCORE then
        self.lab:setString("等待他人加注:" ..  self.timerCount)
    elseif self.gameState == OxsixChangeDefine.SUB_S_CHANGE_CARD then
        self.lab:setString("等待他人换牌:" ..  self.timerCount)
    elseif self.gameState == OxsixChangeDefine.SUB_S_CHANGE_OPEN then
        self.lab:setString("等待他人开牌:" ..  self.timerCount)
    elseif self.gameState == OxsixChangeDefine.SUB_S_CALL_OTH then
        self.lab:setString("等待他人叫庄:" ..  self.timerCount)
    elseif self.gameState == OxsixChangeDefine.SUB_S_CALL_unChange then
        self.lab:setString("请选择你要的换的卡牌:" ..  self.timerCount)
    elseif self.gameState == OxsixChangeDefine.TIME_USER_OPEN_ING then
        self.lab:setString("开牌:" ..  self.timerCount)

    end
end

function OxsixChangeNew:resetGameView()
    --更新玩家试图
...
function OxsixChangeNew:setUserPlayingStatus(wChairID,statu)
    print("wChairID" .. wChairID)
    if wChairID <= OxsixChangeDefine.GAME_PLAYER then
...
function OxsixChangeNew:ShowScore(wChairID,isshow)
    self.bShowSocre[wChairID]=isshow
    self:refreshGameView()
end

function OxsixChangeNew:updateAddSourceView(ishow)
    self.lUserMaxScore = {}
...
function OxsixChangeNew:setUserTableScore(wViewChairID,lAddScoreCount)
...
function OxsixChangeNew:fntCardData(para)
    local data = para
    --dump(data)
    self.cbHandCardData = {}
    local ix = 1
    ...
    for key, var in ipairs(data) do
        ...
        if key % OxsixChangeDefine.MAXCOUNT == 0 then
            ix = ix + 1
            iy = 1
        else
            iy = iy + 1
...
function OxsixChangeNew:dealCardRes(data)
    local index = 1
    self:fntCardData(data)
    self:StartDisPachCard()
end

function OxsixChangeNew:dispatchUserCard(wChairID,cbCardData)
    local wViewChairID=self.wViewChairID[wChairID]
    local poker
...
function OxsixChangeNew:moveDisPachCard()
    
    print("移动卡牌")
    dump(self.cbPlayStatus)
    local winUser = self.wWinUser or 0
    local index = 0
    for j=winUser ,winUser + OxsixChangeDefine.GAME_PLAYER -1 do
        local w= j%OxsixChangeDefine.GAME_PLAYER
        local wViewChairID = self.wViewChairID[w]
        if self.cbPlayStatus[w] == OxsixChangeDefine.USEX_PLAYING then
            for i=1,OxsixChangeDefine.MAXCOUNT do
                local poker
                if wViewChairID == OxsixChangeDefine.MYSELF_VIEW_ID then
                    print("ww.. " .. w)
...
--发牌完成
function OxsixChangeNew:OnSendCardFinish()
    local wMeChairID=self:GetMeChairID()
    if self:IsNotLookonMode()then
        self.changeView:show()
        self.handCardControl[OxsixChangeDefine.MYSELF_VIEW_ID]:SetShow(true)
    end
    ...
    if not self.t_Change and self:IsNotLookonMode() then
        ...
        self.t_Change = oxui.schedule(function () self:updateState()end ,
            OxsixChangeDefine.TIME_INTERVAL,OxsixChangeDefine.TIME_USER_CHANGE_CARD)
    end
end

function OxsixChangeNew:IsNotLookonMode()
    local state
    if self:getMyStatus() == US_PLAYING then
...
function OxsixChangeNew:getMyStatus()
    return self.m_GameUser.myHero.cbUserStatus
end

function OxsixChangeNew:GetMeChairID()
    return self.m_GameUser.myHero.wChairID
end

function OxsixChangeNew:IsCurrentUser(wCurrentUser)
...
function OxsixChangeNew:send(type,data,name)
    self.ClientKernel:requestCommand(MDM_GF_GAME,type,data ,name)
end


function OxsixChangeNew:onOxEnable(isShoot,wchairid)
    ...
    --    print("wchairid =="..wchairid)
    --    dump(cbCardData)
    local newData = GameLogic:sortCardRes(cbCardData,OxsixChangeDefine.MAXCOUNT)
    if myControl then
...
-- 2 是红色云
-- 3是小手
function OxsixChangeNew:addAnimChangeView(type,isLoop)
    local loop = isLoop
    ...
    if t == OxsixChangeDefine.animHead then
