...
function GameclientKernel:registerPlayHandlers()
	...
	{	
		...
		--游戏开始消息返回处理
		{
          ...
          subCmdID = OxsixChangeDefine.SUB_S_GAME_START,
		  responseHandler = handler(self, self.OnSocketSubGameStart)
		},
		--游戏加注消息返回处理
		{
			mainCmdID=MDM_GF_GAME, 
			subCmdID = OxsixChangeDefine.SUB_S_ADD_SCORE, 
			responseHandler = handler(self, self.OnSocketSubGameAddScore)
		},
		--设置强退返回处理
		{
			mainCmdID=MDM_GF_GAME, 
			subCmdID = OxsixChangeDefine.SUB_S_PLAYER_EXIT, 
			responseHandler = handler(self, self.OnSocketSubPlayerExit)
		},
		--游戏结束返回处理
		{
			mainCmdID=MDM_GF_GAME, 
                subCmdID = OxsixChangeDefine.SUB_S_GAME_END, 
                responseHandler = handler(self, self.OnSocketSubGameOver)
		},
		--用户摊牌信息返回处理
		{
			mainCmdID=MDM_GF_GAME, 
			subCmdID = OxsixChangeDefine.SUB_S_OPEN_CARD, 
			responseHandler = handler(self, self.OnSocketSubOpenCard)
		},
		--叫庄返回处理
		{
			mainCmdID=MDM_GF_GAME, 
			subCmdID = OxsixChangeDefine.SUB_S_CALL_BANKER, 
			responseHandler = handler(self, self.OnSocketSubCallBanker)
		},
		--发送返回处理
		{
			mainCmdID=MDM_GF_GAME, 
			subCmdID = OxsixChangeDefine.SUB_S_GAME_BASE, 
			responseHandler = handler(self, self.OnSocketSubBase)
		},
		--用户换牌
		{
			mainCmdID=MDM_GF_GAME, 
                subCmdID = OxsixChangeDefine.SUB_S_CHANGE_CARD, 
			responseHandler = handler(self, self.OnSocketSubChangeCard)
		},

     --发牌
        {
            mainCmdID=MDM_GF_GAME, 
            subCmdID = OxsixChangeDefine.SUB_S_SEND_CARD, 
        ...
        {
            mainCmdID=MDM_GF_GAME, 
                subCmdID = OxsixChangeDefine.SUB_S_CHANGE_OPEN, 
            responseHandler = handler(self, self.OnSocketSubChangeOpen)
        },
        --返回所有的卡牌
        {
            mainCmdID=MDM_GF_GAME, 
            subCmdID = OxsixChangeDefine.SUB_S_USER_OPEN, 
...
function GameclientKernel:OnSocketSubPlayerOpen(Params)
    self.App.EventCenter:dispatchEvent({
        name = OxsixChangeDefine.GAME_PLAYER_OPEN,
        para = Params,
    }) 
    --dump(Params)
end  
...
function GameclientKernel:OnSocketSubGameScene(Params)
    self.App.EventCenter:dispatchEvent({
            name = OxsixChangeDefine.GAME_SCENCE,
            para = Params,
        }) 
    --dump(Params)
end  
...
function GameclientKernel:OnSocketSubGameStart(Params) 
        self.App.EventCenter:dispatchEvent({
            name = OxsixChangeDefine.GAME_START,
            para = Params,
        })  
...
function GameclientKernel:OnSocketSubGameAddScore(Params)
     self.App.EventCenter:dispatchEvent({
            name = OxsixChangeDefine.GAME_ADD_SCORE,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubPlayerExit(Params)
    --print("玩家强制退出Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxsixChangeDefine.GAME_PLAYER_EXIT,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubBase(Params)
    print("发送基数？")
    self.App.EventCenter:dispatchEvent({
            name = OxsixChangeDefine.GAME_BASE,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubChangeOpen(Params)
    print("摊牌Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxsixChangeDefine.GAME_CHANGE_OPEN,
            para = Params,
        }) 
    for k,v in pairs(Params) do
        print(k,v)
...
function GameclientKernel:OnSocketSubOpenCard(Params)
    print("开牌Socket")
    self.App.EventCenter:dispatchEvent({
        name = OxsixChangeDefine.GAME_OPEN_CARD,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubChangeCard(Params)
    --print("换牌Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxsixChangeDefine.GAME_CHANGE_CARD,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubCallBanker(Params)
    --print("叫庄Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxsixChangeDefine.GAME_CALL_BANKER,
            para = Params,
        })
...
function GameclientKernel:OnSocketSendCard(Params)
    --print("发牌Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxsixChangeDefine.GAME_SEND_CARD,
            para = Params,
        })
...
function GameclientKernel:OnSocketSubGameOver(Params)
    --print("结束Socket")
    self.App.EventCenter:dispatchEvent({
            name = OxsixChangeDefine.GAME_OVER,
            para = Params,
        })
