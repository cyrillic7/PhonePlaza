...
local userPos = {
    ...
    cc.p(display.cx + 100,display.cy + 180),--2号位置 对家
    cc.p(display.cx + 190,display.cy),--3号位置 对家
    cc.p(90,190),--4号位置 自己
    cc.p(display.cx - 420,display.cy),--5号位置 对家
    cc.p(display.cx - 330,display.cy + 180),--6号位置 对家
}
...
function Player:ctor(Json,Kernel)
    ...
    for var=1, OxsixChangeDefine.GAME_PLAYER do
...
--    dump(evt.para)
    ...
    if ViewID==OxsixChangeDefine.MYSELF_VIEW_ID then
        --头像框-自己
...
function Player:setCellScore(wChairID,score)
    ...
    if wChairID == OxsixChangeDefine.MYSELF_VIEW_ID then 
...
function Player:setSouceTexture(vid,iszheng)
    ...
    self["TXT" .. vid]:setProperty("0", "oxsixchangenew/Font/" .. str ..  ".png", 23, 31, "0")
