--
-- Author: SuperM
-- Date: 2015-11-06 10:11:59
--
local XWWidgetBase = require("plazacenter.widgets.XWWidgetBase")
local HallBankWidget = class("HallBankWidget", XWWidgetBase)

HallBankWidget.BANKTELLER = 1
HallBankWidget.BANKSAVE = 2
HallBankWidget.BANKTRANSFER = 3
HallBankWidget.BANKINSUREPWD = 4
HallBankWidget.BANKTRANLOG = 5
HallBankWidget.BANKQUERYINSURE = 6

-- option type
HallBankWidget.OptType={
...
function HallBankWidget:ctor(parentNode,szInsureOrgPwd,callBack)
	HallBankWidget.super.ctor(self)
...
function HallBankWidget:onEnter()
    if not self.bFirstEnter then
        return
    end
	HallBankWidget.super.onEnter(self)
...
function HallBankWidget:onCleanup()
	HallBankWidget.super.onCleanup(self)
...
function HallBankWidget:cleanPlistRes()
	print("HallBankWidget:cleanPlistRes")
    display.removeSpriteFramesWithFile("UIHallBank.plist", "UIHallBank.png")
end

function HallBankWidget:getFormatNumberStr(number)
	local strNumber = string.gsub(string.formatnumberthousands(number), ",", ":")
	return strNumber
end

function HallBankWidget:SwitchScoreString(lScore)
...
function HallBankWidget:updateGoldEditAndUpperCase(edit,labUppser,number)
...
function HallBankWidget:updateStorePanel()
...
function HallBankWidget:updateTakeOutPanel()
	if self.st_labelGoldName then
...
function HallBankWidget:updateTransferPanel()
...
function HallBankWidget:updateModifyPanel()
	-- body
end

function HallBankWidget:updateWidgetData()
...
function HallBankWidget:onOptionsSelectChanged(event)
...
function HallBankWidget:onEdit(event, editbox)
...
function HallBankWidget:createMainOption(optType)
	local images={
...
function HallBankWidget:initMainOptions(parentNode)
	if parentNode then
		-- 创建CheckBoxGroup
...
function HallBankWidget:initStoreOrTakeOut(parentNode)
	if parentNode then
...
function HallBankWidget:initTransfer(parentNode)
	if parentNode then
...
function HallBankWidget:initModifyPwd(parentNode)
	if parentNode then
...
function HallBankWidget:onQuitWidget()
	self.callBack()
    self:removeFromParent()
end

function HallBankWidget:onStoreOrOutGoldQuickBtnClicked(goldNumber)	
	-- 存入
	if self.group.currentSelectedIndex_ == 1 then
...
function HallBankWidget:onStoreOrTakeOutBtnClicked()
...
function HallBankWidget:onCheckBoxClicked(clickedBtn,otherBtn)
...
function HallBankWidget:onTransferGoldQuickBtnClicked(goldNumber)
	if goldNumber > GlobalUserInfo.lUserInsure or goldNumber < 0 then
		goldNumber = GlobalUserInfo.lUserInsure
	end
...
function HallBankWidget:onOkTransferBtnClicked()
...
function HallBankWidget:onModifyPwdOkBtnClicked()
...
function HallBankWidget:PerformQueryInfo()
	self:connectLoginServer(self.BANKQUERYINSURE)
end

function HallBankWidget:PerformTakeScore(lTakeScore)
	self.m_lScore = lTakeScore

	self:connectLoginServer(self.BANKTELLER)
end

function HallBankWidget:PerformSaveScore(lSaveScore)
	self.m_lScore = lSaveScore

	self:connectLoginServer(self.BANKSAVE)
end

function HallBankWidget:PerformTransferScore(cbByNickName,szNickName,lTransferScore)
	self.m_lScore = lTransferScore
	self.m_cbByNickName = cbByNickName
	self.m_szNickName = szNickName
	
	self:connectLoginServer(self.BANKTRANSFER)	
end

function HallBankWidget:ModifyInsurePwd(szOldPwd,szNewPwd)
...
function HallBankWidget:connectLoginServer(missionType)
    self.MissionType = missionType

    if not self.missionItem then
        self.missionItem = require("plazacenter.controllers.MissionItem").new(CLIENT_TYPE_LOGIN_POINT,"LoginServer_Bank")
        ...
        self.missionItem.scriptHandler:registerResponseHandler(MDM_GP_USER_SERVICE, SUB_GP_USER_INSURE_INFO, handler(self, self.onGetInsureInfoMessage))
        self.missionItem.scriptHandler:registerResponseHandler(MDM_GP_USER_SERVICE, SUB_GP_USER_INSURE_SUCCESS, handler(self, self.onGetInsureSuccessMessage))
        self.missionItem.scriptHandler:registerResponseHandler(MDM_GP_USER_SERVICE, SUB_GP_USER_INSURE_FAILURE, handler(self, self.onGetInsureFailureMessage))
        self.missionItem.scriptHandler:registerResponseHandler(MDM_GP_USER_SERVICE, SUB_GP_QUERY_USER_INFO_RESULT, handler(self, self.onGetUserInfoResultMessage))
    end
    if self.missionItem then
...
function HallBankWidget:receiveConnectMessage(Params)
   if Params.bConnectSucc then
        print("HallBankWidget bConnectSucc: 连接成功！")
...
function HallBankWidget:receiveShutDownMessage(Params)
   print("HallBankWidget receiveShutDownMessage")
   self:hideLoadingWidget()
end

function HallBankWidget:onOperateSuccessMessage(Params)
    self:hideLoadingWidget()
    ...
    if self.MissionType == self.BANKINSUREPWD then
    	self.szInsureOrgPwd = cc.Crypto:MD5(self.m_szNewInsurePass,false)
    end
end

function HallBankWidget:onOperateFailureMessage(Params)
    self:hideLoadingWidget()
...
function HallBankWidget:onGetInsureInfoMessage(Params)
    self:hideLoadingWidget()
...
function HallBankWidget:onGetInsureSuccessMessage(Params)
    self:hideLoadingWidget()
...
function HallBankWidget:onGetInsureFailureMessage(Params)
    self:hideLoadingWidget()
...
function HallBankWidget:onGetUserInfoResultMessage(Params)
    self:hideLoadingWidget()
    ...
    local dataMsgBox = {
        ...
        callBack=function(ret)
            if ret == MSGBOX_RETURN_OK then
                local CMD_GP_UserTransferScore = {
					dwUserID=GlobalUserInfo.dwUserID,
					cbByNickName=self.m_cbByNickName,
				...
				self.missionItem:requestCommand(MDM_GP_USER_SERVICE, SUB_GP_USER_TRANSFER_SCORE, 
            		CMD_GP_UserTransferScore,"CMD_GP_UserTransferScore")
            else                    
                if self.missionItem then
			        self.missionItem:onDisconnectSocket()
			    end
